component displayname="PeopleDao" accessors="true" extends="base.models.dao" singleton {
	...
	/**
	 * Create a new person from a reference bean
	 *
	 * @referenceBean people.models.PeopleBean     person bean pre-populated with data
	 */
	...
	package query function filter(
		...
		var sql = "SELECT #arguments.returnColumns# FROM person WHERE 1 = 1 ";
	...
	package void function delete(required any referenceBean) {
		if (!arguments.referenceBean.getId() > 0) {
			throw(type="validation", message="Invalid arguments.", detail="person id not found.");
		}
