	...
	function run(){
		describe( "UserService", function(){
			beforeEach( function( currentSpec ){
				setup();				
				variables.model = prepareMock(getInstance( "models.UserService" ));
			} );
			...
			it( "can get a valid mock user by username", function(){
				var peopleObj = people.getBean().setId(1);
				people.$(method = 'loadByEmail', returns = peopleObj);				
				model.$property(propertyName = 'people', mock = people);
